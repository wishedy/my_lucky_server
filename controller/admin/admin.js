...
import { checkReqData, isDataExist } from '../../utils/common'
import { verifyRegister } from '../../verify/admin'
...
class Admin extends AddressComponent {
  ...
  async register (req, res, next) {
    checkReqData(req).then(async (req) => {
      try {
        const info = await isDataExist({
          model: AdminModel,
          param: {
            user_name: req.realName,
            identity_num: req.identityNum
          }
        })
        console.log(info)
        const resultData = verifyRegister(req)
        AdminModel.create(resultData.result)
